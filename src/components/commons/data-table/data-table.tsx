...
export type ColumnsFilterOptionsType = FilterOptionItemType<
  Record<string, any>
>[];
...
interface DataTableProps<T extends { id: number | string }> {
  ...
  source: string;
  rightContent?: (table: any) => React.ReactNode;
}
...
export default function DataTable<T extends { id: number | string }>({
  ...
  React.useEffect(() => {
    if (sorting.length > 0) {
      const sortValue = `${sorting[0]?.desc === false ? "-" : ""}${
        sorting[0]?.id
      }`;
      const newParams = { ...params, ...{ sort: sortValue } };
      setParams(newParams);
    }
  }, [sorting]);
  ...
  React.useEffect(() => {
    const newParams = {
      ...params,
      ...{ search: search, page: search ? 1 : pagination.pageIndex + 1 },
    };
    setParams(newParams);
  }, [search]);
    ...
    <>
      <div className="w-full grid gap-2">
        <Toolbar
          ...
          rightContent={rightContent ? rightContent(table) : undefined}
        />
